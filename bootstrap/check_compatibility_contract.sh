...
# in node-common.sh and control-plane.sh - the three are hand-maintained
...
FAIL=0
if ! grep -q "CLUSTERDRILL_BOOTSTRAP_VERSION=\"${CONTRACT_VERSION}\"" "${SCRIPT_DIR}/control-plane.sh"; then
  echo "compatibility.json's app.version (${CONTRACT_VERSION}) does not match bootstrap/control-plane.sh's CLUSTERDRILL_BOOTSTRAP_VERSION" >&2
  FAIL=1
fi
